...
string handle_request(const string& request, bool should_close) {
    ...
    auto add_connection_close = [&](string& resp) {
        if (should_close) {
            size_t header_end = resp.find("\r\n\r\n");
            if (header_end != string::npos) {
                resp.insert(header_end, "\r\nConnection: close");
            }
        }
    };
    string response;
...
void handle_client(int client_fd) {
    ...
    while (true) {
        ...
        string response = handle_request(request, should_close);
        send(client_fd, response.c_str(), response.length(), 0);
        if (should_close) {
            cout << "Closing connection as requested." << endl;
            break;
        }
